## Scope

### A scope is the range within the program for which the item is valid

和訳「スコープとは、プログラム内において、ある項目（変数や関数など）が有効である範囲のことです。」

次のcodeを実行すると

```rust
// Fix the error below with least amount of modification
fn main() {
    let x: i32 = 10;
    {
        let y: i32 = 5;
        println!("Inner scope value of x is {} and value of y is {}", x, y);
    }
    println!("Outer scope value of x is {} and value of y is {}", x, y);
}
```

以下のようなerror

```shell
   Compiling scope v0.1.0 (/home/doxaripo/projects/rust_by_practice/scope)
error[E0425]: cannot find value `y` in this scope
 --> src/main.rs:8:70
  |
8 |     println!("Outer scope value of x is {} and value of y is {}", x, y);
  |                                                                      ^
  |
help: the binding `y` is available in a different scope in the same function
 --> src/main.rs:5:13
  |
5 |         let y: i32 = 5;
  |             ^

For more information about this error, try `rustc --explain E0425`.
error: could not compile `scope` (bin "scope") due to 1 previous error

[Process exited 101]
```

和訳してみる

- error[E0425]: cannot find value `y` in this scope
- スコープ内でyというvalueを見つけられない。
- value = 「値」「変数」
- help: the binding `y` is available in a different scope in the same function
- 束縛された`y`を同じ変数として異なるscopeで有効にすることが解決策ではなく`y`という名前の束縛自体は別の場所にある。ということ。
  外に出したら解決したがerrorの説明も見る

  解決したcode

```rust
fn main() {
    let x: i32 = 10;
    let y;
    {
        y = 32;
        println!("Inner scope value of x is {} and value of y is {}", x, y);
    }
    println!("Outer scope value of x is {} and value of y is {}", x, y);
}
```

#### エラーの説明

---

An unresolved name was used.(未解決の名前が使われている)

Erroneous code examples:(誤ったコードの例)

```rust
something_that_doesnt_exist::foo;
// error: unresolved name `something_that_doesnt_exist::foo`
//存在しないモジュールからfooという中身を実行しようとしている

// or:

trait Foo {
    fn bar() {
        Self; // error: unresolved name `Self`
    }
}
// trait = 他の型が実装すべき「共通の振る舞い（メソッド）」を定義するためのインターフェースのようなもの。ここでは`Foo`という名前のtraitを作っている。

//fn bar(){...}= そのtraitを実装した型が共通でつかえる。「初期の振る舞い」を定義できる特に指定がなければ、みんなこの`bar`を使ってねっという意味(詳しくは補足へ)
//Self = そのtraitを実装している型自身を指す型別名 なぜエラーになるかはselfは型であって実行できる値や変数ではないから。

// or:

let x = unknown_variable;  // error: unresolved name `unknown_variable`そのまま
```

Please verify that the name wasn't misspelled and ensure that the
identifier being referred to is valid for the given situation. Example:

```rust
enum something_that_does_exist {
    Foo,
}
//something_that_does_existという名前の新しい型を作り、その中にFooという選択肢(値)を用意した。(補足参照)


Or:
mod something_that_does_exist {
    pub static foo : i32 = 0i32;
}

something_that_does_exist::foo; // ok!
//mod something_that_does_exist{ ... }=Moduleの定義。以前は存在しなかった「something_that_does_existという名前の空間」を、今この場で作成した。
//

Or:

let unknown_variable = 12u32;
let x = unknown_variable; // ok!これはわかる。
```

If the item is not defined in the current module, it must be imported using a
`use` statement, like so:(もし現在のモジュールで定義されてないものがあるなら、それはuse statementを使ってインポートしなくてはならない。)

```rust
use foo::bar;
bar();
```

If the item you are importing is not defined in some super-module of the
current module, then it must also be declared as public (e.g., `pub fn`).(現在のモジュールのいくつかの親モジュールで定義されていないあなたがインポートしているものがあるなら、そのときpublicとして宣言しなくてはならない。)

---

#### 補足(Gemini)

#### 「「共通の振る舞い（メソッド）」を定義するためのインターフェースがよくわからない」という自分の問いに対するgeminiの返答

「インターフェース」や「共通の振る舞い」という言葉は、プログラミング独特の抽象的な表現なので、最初はピンとこなくて当然です！

身近な例として、**「リモコン」**を想像してみると分かりやすくなります。

---

1. 「インターフェース（トレイト）」を現実で例えると？

世の中には、テレビ、エアコン、照明など、色々な家電がありますよね。これらは全く別物ですが、共通して**「スイッチを押せば動く」**という振る舞いを持っています。

ここで、**「電源ボタンがあるもの」**という共通のルールを定義したとします。これがプログラミングでいう**トレイト（インターフェース）**です。

- **トレイトの名前:** `電源操作`
- **共通のメソッド:** `スイッチを入れる()`

---

2. なぜこれが必要なの？（メリット）

もし「電源操作」という共通のルールがないと、プログラムを書くときにこうなります。

- テレビを付けるときは `tv.turn_on_tv()`
- エアコンのときは `aircon.start_cooling()`
- 照明のときは `light.switch_light_on()`

これでは、新しい家電が増えるたびに、専用の呼び出し方を覚え直さなければなりません。

しかし、トレイト（インターフェース）を使えば、**「中身が何であれ、とにかく『スイッチを入れる()』という名前のボタンさえ持っていればOK！」**という仕組みが作れます。

---

3. コードで見るとどうなる？

Rustのトレイトで書くと、このようになります。

```rust
// 1. 「共通の振る舞い」というルールを決める
trait Power {
    fn switch_on(&self); // 「このボタンを実装してね」という約束
}

// 2. テレビにそのルールを適用する
struct TV;
impl Power for TV {
    fn switch_on(&self) {
        println!("テレビの画面が付きます");
    }
}

// 3. エアコンにそのルールを適用する
struct Aircon;
impl Power for Aircon {
    fn switch_on(&self) {
        println!("エアコンから風が出ます");
    }
}

```

このように、**「TVもAirconも `Power` という同じルール（インターフェース）に従っている」**ので、使う側は相手がTVかAirconかを深く気にせず、ただ `switch_on()` を呼べば良くなります。

---

4. まとめ

- **インターフェース（トレイト）**: 「どんな機能（メソッド）を持っているか」という**契約書**。
- **共通の振る舞い**: 違う種類のデータ（TVやエアコン）であっても、**同じ名前の命令（メソッド）で動かせる**こと。

「中身がバラバラでも、操作方法だけ統一する」というのがインターフェースの真髄です。

この「共通のルール」を使って、複数のものを一気に操作する方法（ジェネリクスなど）について、具体例を見てみますか？

#### 「そのトレイトを実装した型が共通で使える「初期の振る舞い」を定義できます。がよくわからない」という問いに対するgeminiの回答

「初期の振る舞い（デフォルト実装）」は、いわば**「テンプレート（ひな形）」**のようなものです。

先ほどのリモコンの例をもう少し進化させて説明しますね。

---

1. 「デフォルト実装」を現実で例えると？

例えば、**「スマート家電」というトレイト（ルール）**を作るとします。
最近のスマート家電には、だいたい**「スマホに通知を送る」**という機能がついていますよね。

このとき、テレビも、エアコンも、冷蔵庫も、**「通知を送る」という動作自体は全く同じ（スマホにメッセージを飛ばすだけ）**だとします。

- **デフォルト実装がない場合：**
  テレビ、エアコン、冷蔵庫のそれぞれの説明書（実装コード）に、毎回同じ「通知の送り方」を書き込まなければなりません。面倒ですし、書き間違えるかもしれません。
- **デフォルト実装がある場合：**
  トレイト（ルール）側に、あらかじめ「通知の送り方」を書いておきます。すると、各家電は**「特にこだわりがなければ、この共通のやり方を使ってね」**という状態になります。

これが**「初期の振る舞い」**です。

---

2.  コードで見てみる

Rustのコードで、この「初期の振る舞い」をどう書くか見てみましょう。

```rust
trait SmartDevice {
    // 1. 各家電で「中身」が違うので、名前だけ決める（デフォルトなし）
    fn operate(&self);

    // 2. どの家電でも「中身」が同じなので、最初から書いておく（デフォルト実装）
    fn send_notification(&self) {
        println!("スマホに「動作完了」と通知しました（共通機能）");
    }
}

struct RobotVacuum; // お掃除ロボット
impl SmartDevice for RobotVacuum {
    fn operate(&self) {
        println!("掃除を開始します");
    }
    // send_notification は書かなくても、自動的に「共通機能」が使える！
}

```

---

3.  「上書き（オーバーライド）」もできる

もし、特定の家電だけ「特別な通知を送りたい」と思ったら、デフォルトの振る舞いを**上書き**することもできます。

```rust
struct SecurityCamera; // 防犯カメラ
impl SmartDevice for SecurityCamera {
    fn operate(&self) {
        println!("録画中...");
    }

    // デフォルトは使わず、自分専用の動きにする！
    fn send_notification(&self) {
        println!("【緊急】不審者を検知しました！");
    }
}

```

---

まとめ：なぜ「初期の振る舞い」が便利なのか？

- **楽ができる：** 同じコードを何度も書かなくて済む。
- **最低限の保証：** そのトレイトを実装しさえすれば、細かい設定をしなくても「最低限これだけは動く」という状態にできる。

#### Enumの定義

このコードは、Rustにおける**列挙型（Enum）**の定義です。
以前出てきた `something_that_doesnt_exist::foo` （存在しないもの）とは対照的に、今回は「ちゃんと存在する（does exist）」ものとして定義されていますね。

文法構造と、これがプログラムの中でどう機能するかを解説します。

---

1. 文法の分解

- **`enum` (キーワード)**:
  「列挙型」を定義することを宣言しています。列挙型とは、**「いくつかの選択肢の中から、必ずどれか一つになる型」**のことです。
- **`something_that_does_exist` (型名)**:
  この新しいデータ型の名前です。
- **`{ Foo, }` (バリアント)**:
  この型が持ち得る具体的な選択肢です。この場合、選択肢は `Foo` 一つだけですが、カンマ区切りで増やすことができます（例：`Foo, Bar, Baz`）。

---

2. 以前のエラーとの関係

以前のやり取りにあった `something_that_doesnt_exist::foo` がエラーになったのは、定義がなかったからです。今回のコードを書くことで、以下のように呼び出すことが可能になります。

```rust
// 定義があるから、今度は「解決（Resolve）」できる！
let x = something_that_does_exist::Foo;

```

このように、`::` を使って「`something_that_does_exist` という型の中にある `Foo` という値」を指定できるようになります。

---

3. なぜ Enum を使うのか？（「状態」の定義）

Enumは、複数の状態をひとまとめに扱いたいときに非常に便利です。

例えば、信号機をプログラムにするなら、バラバラの変数を作るよりも Enum が最適です。

```rust
enum TrafficLight {
    Red,
    Yellow,
    Green,
}

```

「信号の色」という一つのカテゴリの中に、複数のパターンを閉じ込めることができます。

---

4. 前の `Self` のエラーと繋げて考えると

もし、この `enum` に対してトレイトを実装しようとすると、こうなります。

```rust
trait Check {
    fn is_foo(&self);
}

impl Check for something_that_does_exist {
    fn is_foo(&self) {
        // ここで Self（大文字）を使うと
        // 「something_that_does_exist」という型そのものを指す
    }
}

```

- **`something_that_does_exist`**: 型の名前（設計図）
- **`something_that_does_exist::Foo`**: 具体的な値（実体）

---

まとめ
このコードは、**「`something_that_does_exist` という名前の新しい型を作り、その中に `Foo` という選択肢（値）を用意した」**という状態です。

## 美しくないコード(宣言のち初期化はあまり好ましくないということを[Rustツアーの変数](https://tourofrust.com/03_ja.html)でみた)ので結局のところどんなコードがいいのかGeminiに聞いてみた。

ここまでの、Rustにおける「スコープ」「関数型的な性質」「シャドウイング」についての対話をまとめました。

---

#### 1. エラーの原因と解決策

元のコードでエラーが出たのは、`y` が内側のブロック `{ }` の中で定義されていたため、ブロックを抜けた瞬間に **「寿命（スコープ）」** が尽きて消えてしまったからです。

最小限の修正案

```rust
let y = {
    let y = 5;
    // ...
    y // セミコロンなしで値を「外」に返す
};

```

---

#### 2. Rustの「関数型」っぽい特徴

Rustは「式（Expression）ベース」の言語であり、多くの構文が値を返します。

- **ブロック `{ }` も式**: 最後に `;` を付けない行を書くと、その値がブロック全体の評価値になります。
- **メリット**: `let mut`（可変）を使わずに、複雑な計算結果を直接 `let` で受け取れるため、コードが安全で簡潔になります。
- **感覚**: 「その場で無名関数を実行して結果を得る」ような動きに近いものです。

---

#### 3. シャドウイング（Shadowing）の仕組み

同じ変数名を「再定義」して、古い変数を隠してしまう手法です。

- **再定義の自由**: `let` を使えば、同じ名前で「違う値」や「違う型」を上書きできます。
- **`mut` との違い**: `mut` は「中身を書き換える」イメージですが、シャドウイングは「新しい変数で古い変数を覆い隠す」イメージです。
- **利点**:
- `input_str` や `input_int` のように、似たような名前を大量に作らなくて済む。
- 計算の途中で型が変わっても、一貫した名前を使える。

---

#### 4. 結論：今回のコードで起きたこと

今回の修正では、**「内側のスコープで計算した `y` を、シャドウイングを使って外側の `y` に引き継いだ」** という合わせ技を使いました。

1. 内側の `y` が誕生。
2. ブロックが `y` の中身を「戻り値」として出力。
3. 外側の `let y = ...` がそれを受け取り、新しい `y` として定義。

---

Rustのこの「厳格だけど柔軟な」ルールに慣れると、メモリ管理を意識しつつも、関数型言語のようなスマートな記述ができるようになります。

次は、これらを踏まえて**「所有権（Ownership）」**などのさらに一歩踏み込んだ話に進んでみますか？それとも、具体的なコードの書き換えを試してみますか？
